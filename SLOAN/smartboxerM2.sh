...
do
	...
	#python3 $SUBFOLD2 --weights $SUBFOLD3 --source "${videoarr[i]}" --class 0 --device 1 --imgsz 640 --max-det=100 
done


Totalfilesexp="$(ls ${PWD%%/}/1detection/runs/predict-seg/)"

for filenameexp in ${Totalfilesexp}
do

	SUBFOLD44=${SUBFOLD11}"${filenameexp}"
